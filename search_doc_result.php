
<!DOCTYPE html>
<html lang="en">
<head>
	<?php include 'head.php' ?>
	<script type="text/javascript" src="js/template2.js"></script>
</head>

<body class="site com_content view-featured no-layout no-task itemid-101 page-home">
<div id="main-wrap" class="collapsed">

	<?php include 'header.php' ?>
	<div class="body">
		<div class="banner-inner tech" >
			<div class="container banner-inner__text text-left text-uppercase">
				<h1 class="banner-inner__title">tech support</h1>
			</div>
		</div>

		<div class="container-fluid breadcrumb-wrap hidden-xs clearfix">
			<div class="container">
				<ul class="breadcrumb">
					<li>
						<a href="">Home</a>
					</li>
					<li>
						<a href="">Tech Support</a>
					</li>
					<li>
						Technical Docs
					</li>
				</ul>
			</div>
		</div>

		<div id="content-container">
			<main id="content">

				<div class=" about-us clearfix">
					<div class="container">
						<div class="row">
							<div class="col-md-3 hidden-sm hidden-xs">
								<div class="about-aside">
									<h3 class="about-aside__tit">Tech support</h3>
									<ul class="about-aside__list">
										<li><a href="">Product Support</a></li>
										<li><a href="">Media Resource</a></li>
										<li><a href="">Customer Support</a></li>
										<li><a href="">Dealer Website</a></li>
										<li><a href="">Parts Lookup</a></li>
										<li><a href="">Recall Information</a></li>
										<li class="active"><a href="">Technical Docs</a></li>
									</ul>
								</div>
							</div>
							<div class="contact col-md-9 search-doc">
								<h2 class="contact__title about">
									Technical Docs
								</h2>
								<div class="search-doc__form-block">
									<form>
										<div class="contact__headline clearfix">
											<h3 class="tit">document search</h3>
										</div>
										<div class="search-field-row">
											<input type="search" class="field no_shadow" value="t235" placeholder="enter model number">
										</div>
										<div class="radio-group">
											<h4 class="radio-group__tit">Language:</h4>
											<div class="item-radio ">
												<label class="radio_item">
													<input type="radio" class="radio_inp" checked name="language">
													<span class="radio_ico"></span>
													<span class="txt">English</span>
												</label>
											</div>
											<div class="item-radio ">
												<label class="radio_item">
													<input type="radio" class="radio_inp" name="language">
													<span class="radio_ico"></span>
													<span class="txt">Spanish</span>
												</label>
											</div>
											<div class="item-radio ">
												<label class="radio_item">
													<input type="radio" class="radio_inp" name="language">
													<span class="txt">All</span>
												</label>
											</div>
										</div>
										<div class="btn-row text-center">
											<input type="submit" class="btn btn-red" value="document search">
										</div>
									</form>
									<div class="search-doc-result">
										<div class="search-doc-result__item">
											<div class="search-doc-result__head">
												Accessory/Attachment Documents
											</div>
											<div class="search-doc-result__sub-item">
												<p><a href="">View Model Accessory/Attachment </a></p>
											</div>
										</div>
										<div class="search-doc-result__item">
											<div class="search-doc-result__head">
												Operators Manual
											</div>
											<div class="recall__item">
												<h4 class="recall__title"><a href="">Grass Trimmer</a></h4>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Model(s):
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T235</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Serial:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">S95611001001 - S95611999999</span>
														<span class="recall__val-item">S95611001001 - S95611999999</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Language:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">English</span>
													</div>
												</div>
											</div>
											<div class="recall__item">
												<h4 class="recall__title"><a href="">Grass Trimmer</a></h4>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Model(s):
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T235</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Serial:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T76214001001 - T76214999999</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Language:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">English</span>
													</div>
												</div>
											</div>
											<div class="recall__item">
												<h4 class="recall__title"><a href="">GRASS TRIMMER / BRUSH CUTTER</a></h4>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Model(s):
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T235</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Serial:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T46714001001-T4671499999</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Language:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">English</span>
													</div>
												</div>
											</div>
										</div>
										<div class="search-doc-result__item">
											<div class="search-doc-result__head">
												Parts Catalog
											</div>
											<div class="recall__item">
												<h4 class="recall__title"><a href="">Grass Trimmer</a></h4>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Model(s):
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T235</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Serial:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T76214001001 - T76214999999</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Language:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">English</span>
													</div>
												</div>
											</div>
											<div class="recall__item">
												<h4 class="recall__title"><a href="">Grass Trimmer</a></h4>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Model(s):
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T235</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Serial:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T76214001001 - T76214999999</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Language:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">English</span>
													</div>
												</div>
											</div>
											<div class="recall__item">
												<h4 class="recall__title"><a href="">Fuel Tank Leak</a></h4>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Model(s):
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T235</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Serial:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">T76214001001 - T76214999999</span>
													</div>
												</div>
												<div class="recall__row clearfix">
													<div class="recall__name">
														Language:
													</div>
													<div class="recall__values">
														<span class="recall__val-item">English</span>
													</div>
												</div>
											</div>
											<div class="search-doc-result__item">
												<div class="search-doc-result__head">
													Warranty Statements
												</div>
												<div class="search-doc-result__sub-item">
													<p><a href="">Shindaiwa Emission Control Warranty Statement</a></p>
													<p><a href="">Shindaiwa Limited Warranty Statement</a></p>
													<p><a href="">Shindaiwa EPA + CARB Emission Warranty Statement</a></p>
												</div>
											</div>
										</div>
									</div>
									<div class="other-search-info">
										<h3 class="other-search-info__tit">Other Information</h3>
										<ul class="list_info">
											<li><a href="">Material Data Sheets</a></li>
											<li><a href="">Contact Us For Literature</a></li>
											<li><a href="">Recall Information</a></li>
											<li><a href="">Parts Lookup</a></li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>

			</main>
		</div>

	</div>
	<?php include 'footer.php' ?>
</div>
</body>
</html>
